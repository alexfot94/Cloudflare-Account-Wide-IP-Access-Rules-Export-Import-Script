# === Config: Import Account-Wide IP Access Rules ===
$headers = @{
...
Write-Host "`nStarting import of $total IP access rule(s) at account level..." -ForegroundColor Cyan
...
foreach ($rule in $rules) {
    ...
    Write-Progress -Activity "Importing Rules (Account-Level)" -Status "Processing $counter of $total" -PercentComplete (($counter / $total) * 100)
